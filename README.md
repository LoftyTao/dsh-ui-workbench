...
- **文件查看器**：UTF-8 文本，带行号、语言标识、一键复制和常用语言语法高亮；长行根据查看区自动换行，内容按滚动位置分批读取与挂载。
- **Git 审查**：工作区/最近提交切换、分支引用、变更文件树、增删统计和带行号 diff；支持并排/统一格式、全部行/隐藏未变更行，全部行模式会按当前列宽换行；覆盖已暂存、未暂存与未跟踪文件，并对长 diff 分批渲染。
